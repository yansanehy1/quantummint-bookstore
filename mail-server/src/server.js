...
const { config } = require('./shared_mock/config');
const { errorHandler } = require('./shared_mock/http/errorHandler');
const requestIdMiddleware = require('./shared_mock/middleware/requestId');
...
class QuantumMailServer {
  ...
  async initializeServices() {
    try {
      ...
      // Initialize Mail Logic Service (Bridge for Web API)
      ...
      // Initialize Web API
      const services = {
        mail: this.mailService, // ADDED: Critical bridge service
  ...
  async startServers() {
    try {
      ...
      try {
        ...
        logger.error('Mail Queue failed to start. Queue-dependent features will be unavailable.', queueError);
      }
