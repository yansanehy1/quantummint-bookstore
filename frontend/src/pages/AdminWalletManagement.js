...
const CustomCard = ({ children, className = "" }) => (<div className={`rounded-xl shadow-lg bg-white overflow-hidden transition-all duration-300 ${className}`}>
    {children}
  </div>);
// Helper component for the Button structure (to replace Shadcn Button)
const CustomButton = ({ children, onClick, className = "" }) => (<button onClick={onClick} className={`flex items-center justify-center rounded-lg px-4 py-2 font-semibold transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-offset-2 focus:ring-offset-white disabled:opacity-50 ${className}`}>
    {children}
  </button>);
const mockUsers = [
...
function App() {
    const [activeTab, setActiveTab] = (0, react_1.useState)("credit-debit");
    const [users, setUsers] = (0, react_1.useState)(mockUsers);
    const [transactions, setTransactions] = (0, react_1.useState)(mockTransactions);
    const [searchTerm, setSearchTerm] = (0, react_1.useState)("");
    const [selectedUser, setSelectedUser] = (0, react_1.useState)(null);
    const [transactionForm, setTransactionForm] = (0, react_1.useState)({ type: "credit", currency: "USD", amount: "", reason: "" });
    const [giftForm, setGiftForm] = (0, react_1.useState)({ bookId: "", bookTitle: "", recipientType: "individual", selectedUserId: "", message: "" });
    ...
    const handleCreditDebit = () => {
        ...
        const newTransaction = { id: String(transactions.length + 1), userId: selectedUser.id, userName: selectedUser.name, type: transactionForm.type, currency: transactionForm.currency, amount, reason: transactionForm.reason, date: new Date(), adminName: "Ibrahim Yansaneh" };
    ...
    // --- RENDERING HELPERS ---
    const renderDashboardCard = (title, value, icon, gradientClass) => (<div className={`p-6 rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.02] ${gradientClass} text-white border-0`}>
    ...
    const renderTabButton = (tabName, label, Icon) => (<button onClick={() => setActiveTab(tabName)} className={`flex items-center px-4 md:px-6 py-3 font-medium transition-all duration-300 border-b-4 
        ${activeTab === tabName
    ...
    return (<div className="min-h-screen bg-gray-50 p-4 sm:p-6 font-inter">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {renderDashboardCard("Total Users", users.length.toString(), lucide_react_1.Users, "bg-gradient-to-tr from-blue-500 to-indigo-600")}
          {renderDashboardCard("USD Credited", `$${totalUsdDistributed.toFixed(2)}`, lucide_react_1.TrendingUp, "bg-gradient-to-tr from-green-500 to-teal-600")}
          {renderDashboardCard("SLL Credited", `Le ${totalSllDistributed.toLocaleString()}`, lucide_react_1.TrendingUp, "bg-gradient-to-tr from-purple-500 to-pink-600")}
          {renderDashboardCard("Total Transactions", transactions.length.toString(), lucide_react_1.History, "bg-gradient-to-tr from-yellow-500 to-orange-600")}
        </div>

        {/* Tabs */}
        <div className="flex overflow-x-auto border-b border-gray-200">
          {renderTabButton("credit-debit", "Wallet Transfers", lucide_react_1.Users)}
          {renderTabButton("gift", "Reward Books", lucide_react_1.Gift)}
          {renderTabButton("history", "Transaction Log", lucide_react_1.History)}
        </div>

        {/* --- Tab Content: Credit/Debit --- */}
        {activeTab === "credit-debit" && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <CustomCard className="lg:col-span-2 shadow-2xl border border-gray-100">
              <div className="p-6">
                ...
                <div className="mb-4 relative">
                  <lucide_react_1.Search className="absolute left-3 top-3.5 w-5 h-5 text-gray-400"/>
                  <input type="text" placeholder="Search by name or email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"/>
                </div>
                <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                  {filteredUsers.length > 0 ? (filteredUsers.map((user) => (<div key={user.id} onClick={() => setSelectedUser(user)} className={`p-4 rounded-xl border-2 cursor-pointer transition duration-300 flex justify-between items-center 
                    ...
                    : "border-gray-200 hover:border-gray-400 hover:bg-gray-50"}`}>
                      ...
                      </div>))) : (<div className="text-center py-10 text-gray-500">No users found matching your search.</div>)}
                </div>
              </div>
            </CustomCard>
            ...
            <CustomCard className="shadow-2xl border border-gray-100">
              <div className="p-6">
                ...
                {!selectedUser && (<div className="text-center py-12 text-gray-500 italic">
                        <lucide_react_1.Users className="w-10 h-10 mx-auto mb-3"/>
                ...
                {selectedUser && (<div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="flex gap-4 p-1 bg-gray-100 rounded-xl shadow-inner">
                        <CustomButton onClick={() => setTransactionForm({ ...transactionForm, type: "credit" })} className={`flex-1 ${transactionForm.type === "credit" ? "bg-green-600 text-white shadow-lg ring-green-500" : "bg-transparent text-gray-700 hover:bg-white"}`}>
                          <lucide_react_1.Plus className="w-4 h-4 mr-2"/>
                          Credit
                        </CustomButton>
                        <CustomButton onClick={() => setTransactionForm({ ...transactionForm, type: "debit" })} className={`flex-1 ${transactionForm.type === "debit" ? "bg-red-600 text-white shadow-lg ring-red-500" : "bg-transparent text-gray-700 hover:bg-white"}`}>
                          <lucide_react_1.Minus className="w-4 h-4 mr-2"/>
                          Debit
                        </CustomButton>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <select value={transactionForm.currency} onChange={(e) => setTransactionForm({ ...transactionForm, currency: e.target.value })} className="w-full px-3 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                    ...
                    <div>
                      ...
                      <input type="number" step="0.01" value={transactionForm.amount} onChange={(e) => setTransactionForm({ ...transactionForm, amount: e.target.value })} placeholder="0.00" className="w-full px-3 py-3 border border-gray-300 rounded-xl shadow-sm text-lg font-bold focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"/>
                    </div>

                    {/* Reason Textarea */}
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Reason</label>
                      <textarea value={transactionForm.reason} onChange={(e) => setTransactionForm({ ...transactionForm, reason: e.target.value })} placeholder="Enter reason for transaction (e.g., 'Compensation', 'Adjustment', 'Penalty')..." rows={3} className="w-full px-3 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"/>
                    </div>

                    {/* Submit Button */}
                    <CustomButton onClick={handleCreditDebit} className={`w-full text-white font-bold text-lg py-3 shadow-xl 
        ...
        {/* --- Tab Content: Gift Books --- */}
        {activeTab === "gift" && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Gift Form Panel */}
            <CustomCard className="shadow-2xl border border-gray-100">
              <div className="p-6">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex gap-4 p-1 bg-gray-100 rounded-xl shadow-inner">
                      <CustomButton onClick={() => setGiftForm({ ...giftForm, recipientType: "individual", selectedUserId: "" })} className={`flex-1 py-3 ${giftForm.recipientType === "individual" ? "bg-indigo-600 text-white shadow-lg ring-indigo-500" : "bg-transparent text-gray-700 hover:bg-white"}`}>
                          Individual User
                      </CustomButton>
                      <CustomButton onClick={() => setGiftForm({ ...giftForm, recipientType: "all", selectedUserId: "" })} className={`flex-1 py-3 ${giftForm.recipientType === "all" ? "bg-indigo-600 text-white shadow-lg ring-indigo-500" : "bg-transparent text-gray-700 hover:bg-white"}`}>
                  ...
                  {giftForm.recipientType === "individual" && (<div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Select User</label>
                      <select value={giftForm.selectedUserId} onChange={(e) => setGiftForm({ ...giftForm, selectedUserId: e.target.value })} className="w-full px-3 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="Filter book titles..." value={giftForm.bookTitle} onChange={(e) => setGiftForm({ ...giftForm, bookTitle: e.target.value })} className="w-full px-3 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition mb-3"/>
                ...
                .map((book) => (<button key={book.id} onClick={() => setGiftForm({ ...giftForm, bookTitle: book.title, bookId: book.id })} className={`w-full text-left p-3 rounded-lg transition duration-200 text-sm border-2 
                  ...
                  <div>
                    ...
                    <textarea value={giftForm.message} onChange={(e) => setGiftForm({ ...giftForm, message: e.target.value })} placeholder="Add a personal message for the recipient(s)..." rows={3} className="w-full px-3 py-3 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"/>
                  </div>

                  {/* Send Gift Button */}
                  <CustomButton onClick={handleGift} className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold text-lg py-3 shadow-xl ring-purple-300">
                    <lucide_react_1.Send className="w-5 h-5 mr-3"/>
                    Send Gift
                  </CustomButton>
                </div>
              </div>
            ...
            ].filter(item => item.show !== false).map((item) => (<div key={item.title} className="bg-white p-4 rounded-xl shadow-md border border-gray-100">
        ...
        {/* --- Tab Content: History --- */}
        {activeTab === "history" && (<CustomCard className="shadow-2xl border border-gray-100">
            <div className="p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {transactions.map((transaction, index) => (<tr key={transaction.id} className={`border-b border-gray-100 ${index % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-indigo-50 transition duration-150`}>
