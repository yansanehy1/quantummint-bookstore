      ...
      <main className="container max-w-6xl mx-auto px-4 py-12">
        ...
        <section className="mb-12">
          <div className="flex gap-4 border-b border-gray-200">
            <button onClick={() => setActiveTab("send")} className={`px-6 py-3 font-semibold transition ${activeTab === "send"
            ? "text-amber-600 border-b-2 border-amber-600"
            : "text-gray-600 hover:text-gray-900"}`}>
              <lucide_react_1.Send className="w-4 h-4 inline mr-2"/>
              Send a Gift
            </button>
            <button onClick={() => setActiveTab("sent")} className={`px-6 py-3 font-semibold transition ${activeTab === "sent"
            ? "text-amber-600 border-b-2 border-amber-600"
            : "text-gray-600 hover:text-gray-900"}`}>
              <lucide_react_1.Send className="w-4 h-4 inline mr-2"/>
              Sent Gifts
            </button>
            <button onClick={() => setActiveTab("received")} className={`px-6 py-3 font-semibold transition ${activeTab === "received"
            ? "text-amber-600 border-b-2 border-amber-600"
            : "text-gray-600 hover:text-gray-900"}`}>
              <lucide_react_1.Gift className="w-4 h-4 inline mr-2"/>
              Received Gifts
            </button>
          </div>
        </section>
        ...
        {/* Send Gift Tab */}
        {activeTab === "send" && (<section className="mb-12">
            <card_1.Card className="p-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <select id="book-select" aria-label="Select a book to gift" value={selectedBook} onChange={(e) => setSelectedBook(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg font-medium">
                    <option value="">Choose a book...</option>
                    {mockBooks.map(book => (<option key={book.id} value={String(book.id)}>
                ...
                <div>
                  ...
                  <input_1.Input id="recipient-phone" type="tel" placeholder="+232 76 123 456" value={recipientPhone} onChange={(e) => setRecipientPhone(e.target.value)} className="py-2 h-12"/>
                ...
                <div>
                  ...
                  <textarea id="gift-note" aria-label="Personal message to include with the gift" placeholder="Add a personal message to your gift..." value={giftNote} onChange={(e) => setGiftNote(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg font-medium h-24"/>
                </div>

                {/* Summary */}
                {selectedBook && (<card_1.Card className="p-6 bg-blue-50 border border-blue-200">
                ...
                <button_1.Button onClick={handleSendGift} disabled={!selectedBook || !recipientPhone} className="w-full bg-pink-600 hover:bg-pink-700 py-3 text-lg">
        ...
        {/* Sent Gifts Tab */}
        {activeTab === "sent" && (<section className="mb-12">
            <card_1.Card className="p-8">
              <h2 className="text-2xl font-bold mb-6">Gifts You've Sent</h2>
              
              <div className="space-y-4">
                {mockSentGifts.length > 0 ? (mockSentGifts.map(gift => (<card_1.Card key={gift.id} className="p-6 border-l-4 border-pink-600">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2">
                            {gift.status === "claimed" ? (<>
                                ...
                                <lucide_react_1.Clock className="w-4 h-4 text-yellow-600"/>
                        ...
                        <div className="text-right">
                          ...
                          <button_1.Button variant="outline" size="sm">Resend SMS</button_1.Button>
                        </div>
                      </div>
        ...
        {/* Received Gifts Tab */}
        {activeTab === "received" && (<section className="mb-12">
            <card_1.Card className="p-8">
              <h2 className="text-2xl font-bold mb-6">Gifts You've Received</h2>
              
              <div className="space-y-4">
                {mockReceivedGifts.length > 0 ? (mockReceivedGifts.map(gift => (<card_1.Card key={gift.id} className="p-6 border-l-4 border-cyan-600">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h3 className="font-bold text-gray-900 mb-2">{gift.bookTitle}</h3>
                        ...
                        <div className="text-right">
                          {gift.status !== "claimed" && (<button_1.Button className="bg-cyan-600 hover:bg-cyan-700">Claim Gift</button_1.Button>)}
                        </div>
                      </div>
                    </card_1.Card>))) : (<div className="text-center py-12">
                    <lucide_react_1.Gift className="w-16 h-16 text-gray-300 mx-auto mb-4"/>
