...
import { BookOpen, Headphones, Bookmark, Share2, ShoppingCart, Play, Pause, Volume2, Download, Eye, ThumbsUp, ThumbsDown, Trash2, Edit, Send, MessageSquare, Verified, Star, X } from "lucide-react";
...
const NotificationBar = ({ message, type, onClose }: NotificationBarProps) => {
...
const useToastMock = () => {
  const [toastState, setToastState] = useState({ message: '', type: '', id: 0 });
...
const Button = ({ children, className = "", variant = "default", onClick, disabled, 'aria-label': ariaLabel, 'aria-pressed': ariaPressed, type = "button" }: ButtonProps) => {
  ...
  return (
    <button className={`${baseClasses} ${className}`} onClick={onClick} disabled={disabled} aria-label={ariaLabel} aria-pressed={ariaPressed} type={type}>
      {children}
    </button>
  );
};
...
const Card = ({ children, className = "" }: CardProps) => (
  <div className={`bg-white rounded-2xl shadow-lg transition duration-300 hover:shadow-xl ${className}`}>
    {children}
  </div>
);
...
const StarRating = ({ rating, readOnly = true, size = "md", showLabel = true }: StarRatingProps) => {
  ...
  return (
    <div className="flex items-center gap-1">
      {Array.from({ length: maxStars }).map((_, index) => {
        ...
        return (
          <Star key={index} className={`${starClasses} ${full || half ? 'text-amber-500 fill-amber-500' : 'text-gray-300 fill-gray-100'}`} />
        );
      })}
...
const ReviewWithReplies = ({ review, replies, onDelete, onEdit, onHelpful, onUnhelpful, onReplySubmit, onReplyDelete }: ReviewWithRepliesProps) => {
  ...
  return (
    <Card className="p-6 space-y-4 border border-gray-100">
        ...
        <div className="text-right">
          {review.isVerifiedPurchase && (
            <div className="flex items-center text-xs text-green-600 mb-1">
              <Verified className="w-4 h-4 mr-1" /> Verified Purchase
            </div>
          )}
      ...
      <div className="flex items-center gap-4 text-sm text-gray-600">
        <div className="flex items-center gap-2">
          <Button variant="ghost" className="h-auto p-1 text-gray-600 hover:text-green-600" onClick={() => onHelpful(review.id)} aria-label="Mark review as helpful">
            <ThumbsUp className="w-4 h-4 mr-1" /> ({review.helpfulCount})
          </Button>
          <Button variant="ghost" className="h-auto p-1 text-gray-600 hover:text-red-600" onClick={() => onUnhelpful(review.id)} aria-label="Mark review as unhelpful">
            <ThumbsDown className="w-4 h-4 mr-1" /> ({review.unhelpfulCount})
          </Button>
        </div>
      ...
      {replies.length > 0 && (
        <div className="pl-8 pt-2 border-l-2 border-gray-100 space-y-3">
          {replies.map((reply) => (
            <div key={reply.id} className="text-sm p-3 bg-gray-50 rounded-xl">
              <div className="flex items-center justify-between mb-1">
                ...
                {reply.authorName === "You" && (
                  <Button variant="ghost" className="h-auto p-0.5 text-red-600" onClick={() => onReplyDelete(reply.id)} aria-label="Delete reply">
...
interface BookAccessGateProps {
  ...
  isPurchased: boolean;
  onPurchase: () => void;
  onStartPayPerUse: (type: string) => void;
  bookId?: number;
  bookTitle?: string;
  bookLevel?: string;
}

const BookAccessGate = ({ bookPrice, userBalance, isPurchased, onPurchase, onStartPayPerUse }: BookAccessGateProps) => {
  ...
  return (
    <Card className="p-8 border-4 border-amber-200 bg-amber-50">
...
interface RealTimeSessionTrackerProps {
  ...
  onSessionEnd: (data: { minutes: number; cost: string }) => void;
  bookId?: number;
  bookTitle?: string;
  bookLevel?: string;
  userBalance?: number;
...
const RealTimeSessionTracker = ({ sessionType, onSessionEnd }: RealTimeSessionTrackerProps) => {
  ...
  return (
    <Card className="p-6 bg-blue-50 border-4 border-blue-200">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          <Headphones className={`w-6 h-6 ${sessionType === 'listening' ? 'text-blue-600' : 'hidden'}`} />
          <BookOpen className={`w-6 h-6 ${sessionType === 'reading' ? 'text-blue-600' : 'hidden'}`} />
          <div>
...
const App = () => {
  ...
  const [activeSession, setActiveSession] = useState<{ type: 'reading' | 'listening' } | null>(null);
  ...
  const handleStartPayPerUse = (sessionType: 'reading' | 'listening') => {
    setActiveSession({ type: sessionType });
    info(`Starting new pay-per-use ${sessionType} session.`);
  };

  const handleSessionEnd = (_state: { minutes: number; cost: string }) => {
  ...
  const handleReplySubmit = (reviewId: number, reply: { content: string }) => {
    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <main className="container max-w-6xl mx-auto px-4 py-16">
        ...
        <section className="grid md:grid-cols-3 gap-10 mb-16">
          ...
          <div className="md:col-span-1 flex flex-col items-center md:items-start">
            ...
            <div className="w-full max-w-sm space-y-3">
              <Button
                onClick={() => { success(`Added "${mockBook.title}" to cart.`); setLocation("/checkout"); }}
              ...
              <Button variant="outline" className="w-full py-3"><Download className="w-5 h-5 mr-2" />Download Sample</Button>
              <Button variant="outline" className="w-full py-3"><Share2 className="w-5 h-5 mr-2" />Share Book</Button>
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex flex-wrap items-center gap-6 mb-8">
              ...
              {mockBook.hasAudio && (
                <div className="flex items-center gap-2 bg-blue-100 text-blue-700 px-4 py-2 rounded-full font-semibold shadow-inner">
        ...
        <section>
          ...
          {activeTab === "reader" && (
            <div id="tab-content-reader" role="tabpanel" aria-labelledby="tab-reader" ref={readerPanelRef} tabIndex={-1} className="space-y-10">
              {/* Access Gates and Session Tracking */}
              <div className="space-y-6">
                {activeSession && (
                  <RealTimeSessionTracker bookId={mockBook.id} bookTitle={mockBook.title} bookLevel="SSS" userBalance={userBalance} sessionType={activeSession.type} onSessionEnd={handleSessionEnd} />
                )}

                {!isPurchased && !activeSession && (
                  <BookAccessGate bookId={mockBook.id} bookTitle={mockBook.title} bookLevel="SSS" bookPrice={mockBook.price} userBalance={userBalance} isPurchased={isPurchased} onPurchase={() => { setIsPurchased(true); success("Book purchased successfully!"); }} onStartPayPerUse={handleStartPayPerUse} />
                )}

                {(isPurchased || activeSession) && (
                  <Card className="p-8 bg-green-50 border-4 border-green-200">
                    ...
                    <Button className="bg-green-600 hover:bg-green-700 text-white"><BookOpen className="w-5 h-5 mr-2" />Continue Reading</Button>
                  </Card>
                )}
              </div>
              ...
              <Card className="p-8 border-2 border-gray-100">
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 space-y-8">
                    ...
                    {/* Audio Player */}
                    {mockBook.hasAudio && (
                      <Card className="p-6 bg-gradient-to-r from-blue-50 to-cyan-50 shadow-lg border-2 border-blue-200">
                        <h3 className="text-lg font-bold text-blue-800 mb-4 flex items-center"><Headphones className="w-5 h-5 mr-2" /> Audio Narration</h3>
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1">
                            <div role="progressbar" aria-valuenow={Math.round(((currentTime / (duration || 1)) * 100))} aria-valuemin={0} aria-valuemax={100} className="bg-blue-200 h-2 rounded-full mb-2 cursor-pointer" onClick={(e) => { /* Handle seek here */ }}>
                  ...
                  {showBookmarks && (
                    <div className="lg:col-span-1">
                      <Card className="p-5 h-full bg-amber-50 shadow-inner border border-amber-200">
                        <h3 className="font-bold text-xl mb-4 text-amber-800 flex items-center"><Bookmark className="w-5 h-5 mr-2" /> Your Bookmarks</h3>
                        <div className="space-y-4">
                          {mockBookmarks.length > 0 ? (
                            mockBookmarks.map(bm => (
                              <div key={bm.id} className="border-l-4 border-amber-600 pl-3 py-2 bg-white rounded-md shadow-sm">
                            ...
                            <p className="text-gray-600 text-sm">No bookmarks yet. Click 'Add Bookmark' above!</p>
                          )}
                        </div>
                      </Card>
                    </div>
          ...
          {activeTab === "reviews" && (
            <div id="tab-content-reviews" role="tabpanel" aria-labelledby="tab-reviews" ref={reviewsPanelRef} tabIndex={-1} className="space-y-10">
