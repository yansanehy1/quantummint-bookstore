...
export function getPricePerHour(level: BookLevel): number {
  switch (level) {
    case "JSS":
      return 1.0;
    case "SSS":
...
export function estimateReadingTime(balance: number, level: BookLevel): number {
...
export function calculateCharge(minutes: number, hourlyRate: number): number {
...
export function getBalanceWarning(currentBalance: number, _currentCharge: number, level: BookLevel): string | null {
  const perMinute = getPricePerHour(level) / 60;
