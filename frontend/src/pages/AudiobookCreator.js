...
exports.default = AudiobookCreator;
// frontend/src/pages/AudiobookCreator.tsx
const react_1 = require("react");
const lucide_react_1 = require("lucide-react");
require("katex/dist/katex.min.css");
const react_katex_1 = require("react-katex");
const textAnalyzer_1 = require("../utils/text-analysis/textAnalyzer");
const textAnalyzer_2 = require("../utils/text-analysis/textAnalyzer");
const formulaDetector_1 = require("../utils/text-analysis/formulaDetector");
// Helper components
const GrammarIssuesViewer = ({ issues, onApplySuggestion }) => (<div>
    <h3 className="text-lg font-semibold mb-4">Grammar Issues</h3>
    {issues.length > 0 ? (<div className="space-y-4">
        {issues.map((issue, index) => (<div key={index} className="p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded-r">
            <div className="flex items-start">
              ...
              <div className="ml-3">
                ...
                {issue.suggestion && (<div className="mt-2">
      ...
      </div>) : (<div className="text-center py-8 bg-green-50 rounded-lg">
        <lucide_react_1.CheckCircle className="mx-auto h-12 w-12 text-green-400"/>
        <h3 className="mt-2 text-sm font-medium text-gray-900">No grammar issues found</h3>
      </div>)}
  </div>);
const MathIssuesViewer = ({ issues, text }) => (<div>
    <h3 className="text-lg font-semibold mb-4">Math Expressions</h3>
    {issues.length > 0 ? (<div className="space-y-4">
        {issues.map((math, index) => (<div key={index} className="p-4 bg-blue-50 border-l-4 border-blue-400 rounded-r">
            <div className="flex items-start">
              {math.result.isValid ? (<lucide_react_1.CheckCircle className="h-5 w-5 text-green-500 mt-0.5"/>) : (<lucide_react_1.AlertTriangle className="h-5 w-5 text-red-500 mt-0.5"/>)}
              <div className="ml-3">
                <div className="text-sm font-medium text-gray-900 mb-2">
                  <react_katex_1.InlineMath math={math.expression}/>
                </div>
                {math.result.isValid ? (<div className="text-sm text-green-700">
                    {math.result.verification || 'Valid math expression'}
                    {math.result.solution && (<div className="mt-1">
                        Result: {JSON.stringify(math.result.solution)}
                      </div>)}
                  </div>) : (<div className="text-sm text-red-700">
        ...
        <lucide_react_1.Info className="mx-auto h-12 w-12 text-gray-400"/>
...
const ScienceIssuesViewer = ({ issues, onApplySuggestion }) => (<div>
    <h3 className="text-lg font-semibold mb-4">Scientific Accuracy</h3>
    {issues.length > 0 ? (<div className="space-y-4">
        {issues.map((issue, index) => (<div key={index} className={`p-4 border-l-4 rounded-r ${issue.severity === 'high'
...
function AudiobookCreator() {
    ...
    const [processedSentences, setProcessedSentences] = (0, react_1.useState)([]);
    ...
    const handleProcessText = async () => {
        ...
        setIsAnalyzing(true);
        try {
            const results = await (0, textAnalyzer_1.analyzeText)(currentPage.rawText);
    ...
    return (<div className="container mx-auto p-6">
      ...
      {activeTab === 'preview' && (<div className="mt-8">
          ...
          {isAnalyzing ? (<div className="flex items-center justify-center p-8">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
              <span className="ml-3">Analyzing text...</span>
            </div>) : analysisResults ? (<div className="space-y-6">
              {activeAnalysisTab === 'grammar' && (<GrammarIssuesViewer issues={analysisResults.grammarIssues} onApplySuggestion={(suggestion) => {
              ...
              {activeAnalysisTab === 'math' && (<MathIssuesViewer issues={analysisResults.mathIssues} text={currentPage.rawText}/>)}

              {activeAnalysisTab === 'science' && (<ScienceIssuesViewer issues={analysisResults.scientificIssues} onApplySuggestion={(suggestion) => {
                        const newText = currentPage.rawText.substring(0, suggestion.start) +
                            (suggestion.suggestion || '') +
                            currentPage.rawText.substring(suggestion.end);
                        updateCurrentPage({ rawText: newText });
