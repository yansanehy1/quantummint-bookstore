...
const MathRenderer_1 = __importDefault(require("../MathRenderer"));
...
const Reader = ({ book, onClose }) => {
    ...
    return (<div className="fixed inset-0 bg-white z-50 flex flex-col md:flex-row h-screen w-screen overflow-hidden">
            {/* Close button */}
            <div className="absolute top-4 left-4 z-50">
                <button onClick={onClose} className="bg-white/80 backdrop-blur p-2 rounded-full shadow-md hover:bg-slate-100 transition-transform hover:scale-110">
                    <lucide_react_1.X className="w-6 h-6 text-slate-800"/>
                </button>
            </div>
            ...
            <div className="flex-1 h-1/2 md:h-full overflow-y-auto bg-white p-6 md:p-12 relative border-r border-slate-100 scroll-smooth">
                <div className="max-w-2xl mx-auto pt-12 md:pt-0 pb-32">
                    ...
                    <div className="space-y-6">
                        {book.content.map((segment, idx) => (<div key={segment.id} onClick={() => handleSegmentClick(idx)} className={`
            ...
            <div className="flex-1 h-1/2 md:h-full bg-slate-900 text-white flex flex-col relative overflow-hidden">
                ...
                <div className="flex-1 flex items-center justify-center p-8 relative">
                    ...
                    <div className="relative z-10 w-full max-w-lg text-center transition-all duration-500 transform">
                        ...
                        {/* IMAGE */}
                        {currentSegment?.type === types_1.SegmentType.IMAGE && (<div className="animate-fade-in space-y-4">
                                <img src={currentSegment.visualContent} alt={currentSegment.visualDescription} className="rounded-lg shadow-2xl border-4 border-slate-700/50 mx-auto max-h-[50vh] object-cover"/>
                        ...
                        {/* FORMULA */}
                        {currentSegment?.type === types_1.SegmentType.FORMULA && (<div className="bg-white text-slate-900 p-8 rounded-xl shadow-2xl animate-scale-in">
                                <p className="text-slate-500 text-sm font-bold uppercase tracking-wider mb-4">Core Concept</p>
                                <MathRenderer_1.default formula={currentSegment.visualContent || ''}/>
                                <p className="text-slate-600 mt-4 italic">{currentSegment.visualDescription}</p>
                            </div>)}

                        {/* AUDIO ONLY/TEXT */}
                        {(currentSegment?.type === types_1.SegmentType.TEXT || currentSegment?.type === types_1.SegmentType.STEP) && (<div className="flex flex-col items-center justify-center h-full space-y-6">
                                <div className={`w-24 h-24 rounded-full bg-slate-800 flex items-center justify-center border border-slate-700 ${isPlaying ? 'animate-pulse ring-4 ring-indigo-500/20' : ''}`}>
                                    <lucide_react_1.Volume2 className="w-10 h-10 text-indigo-400"/>
                                </div>
                ...
                <div className="h-24 bg-slate-800 border-t border-slate-700 flex items-center px-8 justify-between relative z-20">
                    ...
                    <div className="flex items-center gap-6 justify-center w-1/3">
                        <button onClick={() => handleSkip('prev')} className="text-slate-400 hover:text-white transition-colors">
                            <lucide_react_1.SkipBack className="w-6 h-6"/>
                        </button>

                        <button onClick={togglePlay} className="w-14 h-14 bg-indigo-500 hover:bg-indigo-400 text-white rounded-full flex items-center justify-center shadow-lg hover:shadow-indigo-500/25 transition-all transform hover:scale-105">
                            {isPlaying ? <lucide_react_1.Pause className="w-6 h-6 fill-current"/> : <lucide_react_1.Play className="w-6 h-6 fill-current pl-1"/>}
                        </button>

                        <button onClick={() => handleSkip('next')} className="text-slate-400 hover:text-white transition-colors">
                            <lucide_react_1.SkipForward className="w-6 h-6"/>
                        </button>
                    </div>

                    <div className="hidden md:block w-1/3 text-right">
                        <div className="w-full bg-slate-700 h-1 rounded-full overflow-hidden mb-1">
                            <div className="bg-indigo-500 h-full transition-all duration-300" style={{ width: `${((currentIndex + 1) / book.content.length) * 100}%` }}/>
                        </div>
