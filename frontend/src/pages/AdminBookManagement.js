...
const Button = ({ children, onClick, variant = "default", size = "default", className = "", disabled = false }) => {
    ...
    return (<button className={`${baseClasses} ${className}`} onClick={onClick} disabled={disabled}>
      {children}
    </button>);
};
// 2. Mock Card
const Card = ({ children, className = "" }) => (<div className={`bg-white border border-gray-200 rounded-xl transition duration-300 ${className}`}>
    {children}
  </div>);
// 3. Mock Input/Textarea
const Input = ({ className = "", ...props }) => (<input className={`w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ${className}`} {...props}/>);
const Textarea = ({ className = "", ...props }) => (<textarea className={`w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ${className}`} {...props}/>);
// 4. Mock Toast/Sonner
const ToastContainer = ({ toasts }) => (<div className="fixed bottom-4 right-4 z-50 space-y-2">
    {toasts.map((toast) => (<div key={toast.id} className={`p-4 rounded-xl shadow-2xl min-w-80 border-l-4 ${toast.type === 'success' ? 'bg-green-50 border-green-600 text-green-800' : 'bg-red-50 border-red-600 text-red-800'} transition duration-300 ease-in-out`}>
        {toast.message}
      </div>))}
  </div>);
...
const Dialog = ({ children, open, onOpenChange }) => {
    ...
    return (<DialogContext.Provider value={{ isOpen, handleOpenChange }}>
      {/* Render Trigger component */}
      {children.filter(child => child.type === DialogTrigger)}

      {/* Modal overlay */}
      {isOpen && (<div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4 transition-opacity duration-300" onClick={() => handleOpenChange(false)}>
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full transform transition-all duration-300 scale-100 opacity-100" onClick={(e) => e.stopPropagation()} // Prevent closing on inner click
        >
...
const DialogTrigger = ({ children, asChild }) => {
    const { handleOpenChange } = react_1.default.useContext(DialogContext);
    const child = react_1.default.Children.only(children);
    return react_1.default.cloneElement(child, { onClick: () => handleOpenChange(true) });
};
...
// 6. Mock Tabs
const TabsContext = react_1.default.createContext({});
...
const TabsList = ({ children, className }) => (<div className={`flex bg-gray-100 p-1 rounded-xl shadow-inner ${className}`}>
    {children}
  </div>);
const TabsTrigger = ({ children, value }) => {
    const { activeTab, setActiveTab } = react_1.default.useContext(TabsContext);
    const isActive = activeTab === value;
    return (<button onClick={() => setActiveTab(value)} className={`flex-1 px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${isActive ? 'bg-white text-blue-700 shadow-md' : 'text-gray-600 hover:text-blue-500'}`}>
      {children}
    </button>);
};
const TabsContent = ({ children, value, className }) => {
    const { activeTab } = react_1.default.useContext(TabsContext);
...
function AdminBookManagement() {
    ...
    const approveBook = (bookId) => {
        setBooks(books.map((b) => (b.id === bookId ? { ...b, status: "approved", rejectionReason: undefined } : b)));
    ...
    const handleOpenRejectDialog = (book) => {
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            case "approved":
                return <lucide_react_1.CheckCircle className="w-4 h-4 mr-1.5"/>;
            case "pending":
                return <lucide_react_1.Clock className="w-4 h-4 mr-1.5"/>;
            case "rejected":
                return <lucide_react_1.XCircle className="w-4 h-4 mr-1.5"/>;
            default:
                return null;
        }
    };
    const BookCard = ({ book }) => (<Card className="p-5 group shadow-lg hover:shadow-2xl transition duration-300 transform hover:-translate-y-1">
      <div className="flex justify-between items-start mb-4">
        ...
        <div className={`px-3 py-1 rounded-full text-xs font-bold border flex items-center whitespace-nowrap ${getStatusColor(book.status)}`}>
      ...
      {book.rejectionReason && (<div className="bg-red-50 border border-red-300 rounded-lg p-3 mb-4 transition duration-300">
          <p className="text-xs font-bold text-red-900 flex items-center mb-1">
            <lucide_react_1.XCircle className="w-4 h-4 mr-2 text-red-600"/>
      ...
      <div className="flex gap-3 pt-2">
        ...
        {book.status === "pending" && (<>
            <Button onClick={() => approveBook(book.id)} size="sm" className="flex-1 bg-green-600 hover:bg-green-700 transition duration-200">
              <lucide_react_1.CheckCircle className="w-4 h-4 mr-2"/>
              Approve
            </Button>

            <Button onClick={() => handleOpenRejectDialog(book)} size="sm" className="flex-1 bg-red-600 hover:bg-red-700 transition duration-200">
    ...
    return (<div className="min-h-screen bg-gray-50 py-10 font-sans">
      <div className="container max-w-7xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-10">
          <Card className="p-5 shadow-xl transition hover:shadow-2xl hover:scale-[1.01] bg-yellow-50 border-yellow-300">
            <div className="flex items-center justify-between">
              ...
              <lucide_react_1.Clock className="w-10 h-10 text-yellow-500 opacity-60"/>
            </div>
          </Card>
          <Card className="p-5 shadow-xl transition hover:shadow-2xl hover:scale-[1.01] bg-green-50 border-green-300">
            <div className="flex items-center justify-between">
              ...
              <lucide_react_1.CheckCircle className="w-10 h-10 text-green-500 opacity-60"/>
            </div>
          </Card>
          <Card className="p-5 shadow-xl transition hover:shadow-2xl hover:scale-[1.01] bg-red-50 border-red-300">
            <div className="flex items-center justify-between">
              ...
              <lucide_react_1.XCircle className="w-10 h-10 text-red-500 opacity-60"/>
            </div>
          </Card>
        </div>
        ...
        <Tabs defaultValue="pending" className="w-full">
          ...
          <TabsContent value="pending" className="space-y-6">
            {pendingBooks.length > 0 ? (<div className="grid lg:grid-cols-3 md:grid-cols-2 gap-6">
                {pendingBooks.map((book) => (<BookCard key={book.id} book={book}/>))}
              </div>) : (<Card className="p-12 text-center shadow-lg">
                <lucide_react_1.Clock className="w-16 h-16 text-yellow-400 mx-auto mb-4"/>
          ...
          <TabsContent value="approved" className="space-y-6">
            {approvedBooks.length > 0 ? (<div className="grid lg:grid-cols-3 md:grid-cols-2 gap-6">
                {approvedBooks.map((book) => (<BookCard key={book.id} book={book}/>))}
              </div>) : (<Card className="p-12 text-center shadow-lg">
                <lucide_react_1.CheckCircle className="w-16 h-16 text-green-400 mx-auto mb-4"/>
          ...
          <TabsContent value="rejected" className="space-y-6">
            {rejectedBooks.length > 0 ? (<div className="grid lg:grid-cols-3 md:grid-cols-2 gap-6">
                {rejectedBooks.map((book) => (<BookCard key={book.id} book={book}/>))}
              </div>) : (<Card className="p-12 text-center shadow-lg">
                <lucide_react_1.XCircle className="w-16 h-16 text-red-400 mx-auto mb-4"/>
        ...
        {selectedBook && (<Dialog open={showRejectDialog} onOpenChange={handleCloseRejectDialog}>
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4 pt-4">
                ...
                <Textarea value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)} placeholder="Explain the issues (e.g., poor formatting, incomplete content, incorrect category) that need to be addressed..." rows={5}/>
                <div className="flex justify-end gap-3 pt-2">
                  <Button onClick={handleCloseRejectDialog} variant="outline" className='text-gray-600 hover:bg-gray-100'>
                    Cancel
                  </Button>
                  <Button onClick={() => rejectBook(selectedBook.id)} className="bg-red-600 hover:bg-red-700" disabled={!rejectionReason.trim()}>
                    <lucide_react_1.XCircle className="w-4 h-4 mr-2"/>
                    Confirm Rejection
                  </Button>
                </div>
