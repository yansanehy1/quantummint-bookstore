...
import { trpc } from "@/lib/trpc"; // Included for completeness, though unused in mock logic
import { useAuth } from "@/_core/hooks/useAuth"; // Included for completeness, though unused in mock logic
...
const MockButton = ({ children, onClick, disabled = false, variant = 'default', className = '' }: any) => (
    <button
        onClick={onClick}
        disabled={disabled}
        className={`px-4 py-2 font-semibold rounded-lg transition-colors duration-200 shadow-md ${
...
const MockInput = ({ value, onChange, placeholder, type = 'text', className = '' }: any) => (
    <input
        type={type}
        value={value}
        onChange={onChange}
        placeholder={placeholder}
        className={`w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition duration-150 ${className}`}
    />
);
...
export default function Checkout() {
  ...
  const StepIndicator = ({ step, currentStep, index }: { step: CheckoutStep, currentStep: number, index: number }) => {
        ...
        <div className="flex flex-col items-center flex-1 relative">
            ...
            {index < steps.length - 1 && (
                <div className={`absolute top-5 left-1/2 w-full h-1 -translate-y-1/2 transition-all duration-500 -z-10 
                    ...
                    style={{ transform: 'translateX(-50%)', width: '100%', left: 'calc(50% + 20px)' }}
                />
            )}
        </div>
    );
  ...
  if (orderPlaced) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-orange-50 py-12 px-4 font-sans">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10 flex justify-between">
            {steps.map((step, index) => (
                <StepIndicator key={step.id} step={step} currentStep={currentStep} index={index} />
            ))}
        </div>
...
const PaymentMethodButton = ({ method, icon, description, selected, onClick }: { method: string, icon: JSX.Element, description: string, selected: boolean, onClick: () => void }) => (
    <button 
        onClick={onClick} 
