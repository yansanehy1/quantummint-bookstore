...
import { BookOpen, TrendingUp, DollarSign, Users, Plus, Edit2, Eye, Trash2, Download, Video, Image as ImageIcon, Code, PlayCircle, Sparkles } from "lucide-react";
...
export const SellerDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<"books" | "earnings" | "analytics" | "studio">("books");
  const [showFullscreenStudio, setShowFullscreenStudio] = useState(false);

  const mockBooks: SellerBook[] = [
    {
      id: 1,
  ...
  const handleGenerateSSML = () => {
    const ssml = buildSSML(ttsInput);
    setGeneratedSSML(ssml);
    setAudioPreviewUrl(null);
  };

  const handleSynthesize = async () => {
    if (!generatedSSML) return;
    setIsSynthesizing(true);
    try {
      const url = await synthesizeSpeech(generatedSSML);
      ...
      <main className="container max-w-6xl mx-auto px-4 py-12">
        <section className="mb-12">
          <div className="flex justify-between items-start mb-8">
            ...
            <Button onClick={() => setShowFullscreenStudio(true)} className="bg-green-600 hover:bg-green-700">
        ...
        <section className="mb-8">
          <div className="flex gap-4 border-b border-gray-200">
            ...
            <button
              ...
              className={`px-6 py-3 font-semibold transition ${activeTab === "studio"
                ? "text-amber-600 border-b-2 border-amber-600"
                : "text-gray-600 hover:text-gray-900"
                }`}
            >
                    ...
                    <Card key={book.id} className="p-6 border-l-4 border-amber-600">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-2">
                          <Button onClick={() => setLocation(`/player`)} variant="outline" size="icon">
                            <Eye className="w-4 h-4" />
                          </Button>
                          <Button onClick={() => setLocation("/book-editor")} variant="outline" size="icon">
                            <Edit2 className="w-4 h-4" />
                          </Button>
                          <Button onClick={() => handleDeleteBook(book.id)} variant="outline" size="icon" className="text-red-600 hover:text-red-700">
            ...
            <Card className="p-8">
              ...
              <div className="space-y-3">
                {mockEarnings.map((earning, idx) => (
                  <div key={idx} className="flex items-center justify-between p-4 border-b border-gray-200 last:border-b-0">
                    <div>
        ...
        {activeTab === "studio" && (
          <section className="mb-12 h-screen">
            <Studio onPreview={(book) => {
              console.log("Preview book:", book);
              alert(`Previewing book: ${book.title}`);
            }} />
          </section>
        )}
      </main>
      ...
      {showFullscreenStudio && (
        <div className="fixed inset-0 z-50 bg-white overflow-auto">
          <Studio
            onPreview={(book) => {
              console.log("Preview book:", book);
              // Store the book in sessionStorage for preview
              sessionStorage.setItem('preview-book', JSON.stringify(book));
              setShowFullscreenStudio(false);
              // Navigate to player with preview book
              setLocation(`/player/preview-${book.id}`);
            }}
