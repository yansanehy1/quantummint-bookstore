...
class QuantumUploadManager extends EventEmitter {
    ...
    async initUpload(metadata) {
        const requiredFields = ['filename', 'size', 'mimeType', 'totalChunks', 'userId'];
        ...
        if (isNaN(size) || size <= 0 || size > this.config.maxFileSize) {
            throw new Error('Invalid file size');
        }
        if (isNaN(totalChunks) || totalChunks <= 0 || totalChunks > 10000) {
            throw new Error('Invalid totalChunks');
        }

        // Only allow known safe MIME types
        const safeMimes = ['video/mp4', 'video/quicktime', 'video/x-msvideo'];
        if (!safeMimes.includes(metadata.mimeType)) {
    ...
    async handleChunk(uploadId, chunkIndex, buffer, userId) {
        let upload = this.activeUploads.get(uploadId);
        if (!upload) {
            // Try to load from disk
            try {
                upload = await this.loadUploadState(uploadId);
            } catch (e) {
        ...
        if (upload.userId !== userId) {
            throw new Error('Unauthorized chunk upload');
        }
        ...
        if (upload.uploadedSize + buffer.length > upload.size) {
    ...
    async finalizeUpload(uploadId) {
        ...
        const sanitizedExt = path.extname(upload.filename).toLowerCase();
        if (!['.mp4', '.mov', '.avi'].includes(sanitizedExt)) {
