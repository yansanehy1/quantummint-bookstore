...
import { Edit2, Save, X, Plus, Trash2, Eye, Clock, BookOpen, DollarSign } from "lucide-react";
...
export default function BookEditor() {
  ...
  const savePage = () => {
    ...
    setEditingPage(null);
    toast.success("Page updated successfully", { description: `Page ${editingPage.pageNumber} (${editingPage.title}) was saved.` });
  };
  ...
  const deletePage = (pageId: number) => {
    ...
    toast.success("Page deleted and re-indexed");
  };
  ...
  const submitForApproval = () => {
    ...
    toast.success("Book submitted for admin approval", { description: "The book is now in 'Pending Approval' status." });
  };
  ...
  const rejectBook = () => {
    ...
    toast.success("Book rejected", { description: "The rejection reason has been recorded." });
    setShowApprovalDialog(false);
    setRejectionReason("");
  };
  ...
  const republish = () => {
    ...
    toast.success("Book resubmitted for approval", { description: "Waiting for admin review." });
  };
      ...
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Tabs defaultValue="details" className="w-full">
          ...
          <TabsList className="grid w-full grid-cols-3 bg-white p-2 rounded-xl shadow-md mb-6 border border-slate-200">
            <TabsTrigger value="details" className="data-[state=active]:bg-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-lg rounded-lg transition-colors">Book Details</TabsTrigger>
            <TabsTrigger value="pages" className="data-[state=active]:bg-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-lg rounded-lg transition-colors">Pages ({book.pages.length})</TabsTrigger>
            <TabsTrigger value="approval" className="data-[state=active]:bg-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-lg rounded-lg transition-colors">Approval & Pricing</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="details" className="space-y-6">
            <Card className="p-6 shadow-xl rounded-xl">
              <div className="flex justify-between items-center mb-6 border-b pb-4">
                ...
                <Button onClick={() => setIsEditingBook(!isEditingBook)} variant={isEditingBook ? "destructive" : "default"} className={isEditingBook ? "shadow-md" : "bg-indigo-600 hover:bg-indigo-700 shadow-md"}>
                ...
                <div className="lg:col-span-2 space-y-5">
                  <div className="grid md:grid-cols-2 gap-5">
                    ...
                    <div>
                      ...
                      <Input type="number" value={book.priceUsd} onChange={(e) => handleBookFieldChange("priceUsd", parseFloat(e.target.value))} disabled={!isEditingBook} className="w-full border-slate-300 focus:border-indigo-500" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-1">Price (SLL)</label>
                      <Input type="number" value={book.priceSll} onChange={(e) => handleBookFieldChange("priceSll", parseFloat(e.target.value))} disabled={!isEditingBook} className="w-full border-slate-300 focus:border-indigo-500" />
                    </div>
                  </div>
                            ...
                            {editingPage && (
                              <div className="space-y-4 pt-4">
                                ...
                                <div className="flex gap-3 pt-4 border-t">
                                  <Button onClick={savePage} className="bg-green-600 hover:bg-green-700 shadow-md">Save Page Changes</Button>
                                  <Button onClick={() => setEditingPage(null)} variant="outline">Cancel</Button>
                                </div>
                              </div>
                            )}
                          </DialogContent>
                ...
                <div className="p-4 rounded-lg border border-slate-300 bg-slate-50 shadow-inner">
                  ...
                  <div className="space-y-2">
                    <p className="flex justify-between items-center font-medium text-slate-600">
                      <span className="flex items-center gap-2"><DollarSign className="w-4 h-4 text-green-600" /> Price (USD):</span>
