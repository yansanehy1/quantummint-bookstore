...
function AdminDashboard() {
    ...
    const [sellerRequests, setSellerRequests] = (0, react_1.useState)([
        {
            ...
            motivation: "I want to share my teaching methods with students across Sierra Leone",
        },
        {
            id: "2",
            name: "Fatima Conteh",
            ...
            motivation: "Passionate about making quality literature education accessible",
        },
        {
            id: "3",
            name: "Ibrahim Koroma",
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            case "pending":
                return <lucide_react_1.Clock className="w-5 h-5 text-yellow-600"/>;
            default:
                return null;
        }
    };
      ...
      <main className="container max-w-7xl mx-auto px-4 py-12">
        ...
        {activeTab === "overview" && (<>
            <div className="mb-8 flex gap-4">
              <button_1.Button onClick={() => setLocation("/admin-book-management")} className="bg-blue-600 hover:bg-blue-700">
                <lucide_react_1.BookOpen className="w-5 h-5 mr-2"/>
                Manage Books
              </button_1.Button>
              <button_1.Button onClick={() => setLocation("/admin-wallet-management")} className="bg-purple-600 hover:bg-purple-700">
            ...
            <card_1.Card className="p-6">
              <div className="flex items-center justify-between">
                ...
                <lucide_react_1.AlertCircle className="w-12 h-12 text-blue-600 opacity-20"/>
              </div>
            </card_1.Card>
            ...
            <card_1.Card className="p-6">
              <div className="flex items-center justify-between">
                ...
                <lucide_react_1.Clock className="w-12 h-12 text-yellow-600 opacity-20"/>
              </div>
            </card_1.Card>
            ...
            <card_1.Card className="p-6">
              <div className="flex items-center justify-between">
                ...
                <lucide_react_1.CheckCircle className="w-12 h-12 text-green-600 opacity-20"/>
              </div>
            </card_1.Card>
            ...
            <card_1.Card className="p-6">
              <div className="flex items-center justify-between">
                ...
                <lucide_react_1.XCircle className="w-12 h-12 text-red-600 opacity-20"/>
              </div>
            </card_1.Card>
            </div>
          </>)}
        ...
        <div className="flex gap-4 mb-8 border-b border-gray-300">
          <button onClick={() => setActiveTab("overview")} className={`px-4 py-2 font-semibold border-b-2 transition ${activeTab === "overview"
            ? "border-amber-600 text-amber-600"
            : "border-transparent text-gray-600 hover:text-gray-900"}`}>
            Overview
          </button>
          <button onClick={() => setActiveTab("requests")} className={`px-4 py-2 font-semibold border-b-2 transition ${activeTab === "requests"
            ? "border-amber-600 text-amber-600"
            : "border-transparent text-gray-600 hover:text-gray-900"}`}>
            Seller Requests
          </button>
          <button onClick={() => setActiveTab("sellers")} className={`px-4 py-2 font-semibold border-b-2 transition ${activeTab === "sellers"
            ? "border-amber-600 text-amber-600"
            : "border-transparent text-gray-600 hover:text-gray-900"}`}>
            Approved Sellers
          </button>
        ...
        {activeTab === "requests" && (<div>
            ...
            <div className="flex gap-4 mb-6">
              <div className="flex-1 relative">
                ...
                <input type="text" aria-label="Search requests" placeholder="Search by name, email, or expertise..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg"/>
              </div>

              <select aria-label="Filter status" value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-4 py-2 border border-gray-300 rounded-lg">
            ...
            <div className="space-y-4">
              {filteredRequests.length === 0 ? (<card_1.Card className="p-8 text-center">
                  <p className="text-gray-600">No requests found</p>
                </card_1.Card>) : (filteredRequests.map((request) => (<card_1.Card key={request.id} className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-lg font-bold text-gray-900">{request.name}</h3>
                          <span className={`flex items-center gap-1 px-3 py-1 rounded-full text-sm font-semibold border ${getStatusColor(request.status)}`}>
                    ...
                    {request.status === "pending" && (<div className="flex gap-3">
                        <button_1.Button onClick={() => handleApprove(request.id)} className="flex-1 bg-green-600 hover:bg-green-700">
                          <lucide_react_1.CheckCircle className="w-4 h-4 mr-2"/>
                          Approve
                        </button_1.Button>
                        <button_1.Button onClick={() => handleReject(request.id)} variant="outline" className="flex-1 border-red-300 text-red-600 hover:bg-red-50">
        ...
        {activeTab === "sellers" && (<>
            <div className="grid md:grid-cols-2 gap-6">
              {sellerRequests
                ...
                .map((seller) => (<card_1.Card key={seller.id} className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                    ...
                    <button_1.Button variant="outline" className="w-full" onClick={() => setLocation(`/seller-profile/${seller.id}`)}>
                      View Profile
                    </button_1.Button>
                  </card_1.Card>))}
            </div>
            ...
            {sellerRequests.filter((r) => r.status === "approved").length ===
                0 && (<card_1.Card className="p-8 text-center">
