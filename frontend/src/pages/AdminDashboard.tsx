...
import { useAuthContext } from "@/_core/contexts/AuthContext";
...
// --- END: Local UI Component Definitions ---
  ...
  const [sellerRequests, setSellerRequests] = useState<SellerRequestItem[]>([
    {
      ...
      motivation:
        "I want to share my teaching methods with students across Sierra Leone and believe my educational materials are top-tier.",
    },
    {
      id: "2",
      name: "Fatima Conteh",
      ...
      motivation:
        "Passionate about making quality literature education accessible to rural students via digital resources.",
    },
    {
      id: "3",
      name: "Ibrahim Koroma",
      ...
      <header className="bg-white shadow-lg sticky top-0 z-50 border-b border-amber-500/30">
        <div className="container max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav className="flex gap-6 items-center">
            ...
            <Users className="w-6 h-6 text-gray-500 cursor-pointer hover:text-amber-600 transition-colors" />
          </nav>
        </div>
      </header>
          ...
          <>
            <div className="mb-8 flex flex-col sm:flex-row gap-4">
              ...
              {user?.role === 'admin' && (
                <Button
                  variant="outline"
                  className="text-sm px-3 py-2 bg-amber-50 border-amber-200 text-amber-700 hover:bg-amber-100"
                  onClick={() => window.open("https://quantum.quantummint.net", "_blank", "noopener,noreferrer")}
                  title="Access restricted to administrators only"
                >
                  Open Quantum Admin
                </Button>
              )}
        ...
        {(activeTab === "requests" || activeTab === "sellers") && (
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8 p-4 bg-white rounded-xl shadow-inner border border-gray-100">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition"
                />
              </div>
              <div className="sm:w-1/3">
                <select
                  ...
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl appearance-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 bg-white cursor-pointer"
            ...
            <div className="space-y-5">
              {filteredRequests.filter(req => activeTab === "sellers" ? req.status === "approved" : true).length === 0 ? (
                ...
                filteredRequests.filter(req => activeTab === "sellers" ? req.status === "approved" : true).map((request) => (
                  <Card key={request.id} className="p-6 transition-all duration-300 transform hover:shadow-2xl hover:border-amber-200">
