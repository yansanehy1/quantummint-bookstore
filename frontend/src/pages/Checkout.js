...
const MockButton = ({ children, onClick, disabled = false, variant = 'default', className = '' }) => (<button onClick={onClick} disabled={disabled} className={`px-4 py-2 font-semibold rounded-lg transition-colors duration-200 shadow-md ${variant === 'outline' ? 'border border-gray-300 text-gray-700 hover:bg-gray-100' :
        'bg-orange-600 text-white hover:bg-orange-700'} ${className}`}>
        {children}
    </button>);
const MockCard = ({ children, className = '' }) => (<div className={`bg-white rounded-xl shadow-lg ${className}`}>
        {children}
    </div>);
const MockInput = ({ value, onChange, placeholder, type = 'text', className = '' }) => (<input type={type} value={value} onChange={onChange} placeholder={placeholder} className={`w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition duration-150 ${className}`}/>);
function Checkout() {
    // Replace imported components with mock versions for runnable single-file context
    const Button = MockButton;
    const Card = MockCard;
    const Input = MockInput;
    const [, setLocation] = (0, wouter_1.useLocation)();
    // Mocking useAuth output for demonstration
    const user = { name: "Adama Sesay", email: "adama.sesay@example.com" };
    // const { user } = useAuth(); 
    const [currentStep, setCurrentStep] = (0, react_1.useState)(1);
    const [selectedCurrency, setSelectedCurrency] = (0, react_1.useState)("SLL"); // Default to SLL
    const [selectedPaymentMethod, setSelectedPaymentMethod] = (0, react_1.useState)("afrimoney"); // Default to local mobile money
    ...
    const processPayment = async () => {
        ...
        try {
            ...
            setOrderPlaced(true);
            // In a real app: call tRPC mutation here
        }
        catch (err) {
    ...
    const StepIndicator = ({ step, currentStep, index }) => {
        const isActive = currentStep === step.id;
        const isCompleted = currentStep > step.id;
        return (<div className="flex flex-col items-center flex-1 relative">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ring-4 ring-offset-2 
                ...
                {isCompleted ? <lucide_react_1.CheckCircle2 className="w-5 h-5"/> : step.id}
            </div>
            <span className={`mt-2 text-sm text-center transition-colors duration-300 
                ${isActive ? "text-gray-900 font-semibold" :
                isCompleted ? "text-green-600 font-medium" :
                    "text-gray-500"}`}>
                {step.title}
            </span>
            {index < steps.length - 1 && (<div className={`absolute top-5 left-1/2 w-full h-1 -translate-y-1/2 transition-all duration-500 -z-10 
                    ${isCompleted ? "bg-green-500" : isActive || currentStep === step.id + 1 ? "bg-orange-300" : "bg-gray-200"}`} style={{ transform: 'translateX(-50%)', width: '100%', left: 'calc(50% + 20px)' }}/>)}
        </div>);
    };
    if (orderPlaced) {
        return (<div className="min-h-screen bg-gradient-to-br from-gray-50 to-orange-50 py-12 px-4 font-sans">
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-gray-50 to-orange-50 py-12 px-4 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10 flex justify-between">
            {steps.map((step, index) => (<StepIndicator key={step.id} step={step} currentStep={currentStep} index={index}/>))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {/* Step 1: Order Review */}
            {currentStep === 1 && (<Card className="p-8 shadow-xl">
                ...
                <div className="space-y-6">
                  ...
                  <div className="pt-4 border-t">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <CurrencyButton currency="USD" price={mockBook.priceUSD} selected={selectedCurrency === "USD"} onClick={() => setSelectedCurrency("USD")}/>
                      <CurrencyButton currency="SLL" price={mockBook.priceSLL} selected={selectedCurrency === "SLL"} onClick={() => setSelectedCurrency("SLL")}/>
                    </div>
                  </div>
                </div>
              </Card>)}
            ...
            {currentStep === 2 && (<Card className="p-8 shadow-xl">
            ...
            {currentStep === 3 && (<Card className="p-8 shadow-xl">
                <h2 className="text-2xl font-bold text-gray-900 mb-6 border-b pb-3">3. Choose Payment Method</h2>
                <div className="space-y-4">
                  <PaymentMethodButton method="Card" icon={<lucide_react_1.CreditCard className="w-5 h-5"/>} description="Visa, Mastercard, Amex" selected={selectedPaymentMethod === "card"} onClick={() => setSelectedPaymentMethod("card")}/>
                  <PaymentMethodButton method="Orange Money" icon={<lucide_react_1.Smartphone className="w-5 h-5"/>} description="Local mobile money" selected={selectedPaymentMethod === "orange"} onClick={() => setSelectedPaymentMethod("orange")}/>
                  <PaymentMethodButton method="Afrimoney" icon={<lucide_react_1.Smartphone className="w-5 h-5"/>} description="Local mobile money" selected={selectedPaymentMethod === "afrimoney"} onClick={() => setSelectedPaymentMethod("afrimoney")}/>
                  <PaymentMethodButton method="Qmoney" icon={<lucide_react_1.Smartphone className="w-5 h-5"/>} description="Local mobile money" selected={selectedPaymentMethod === "qmoney"} onClick={() => setSelectedPaymentMethod("qmoney")}/>
                </div>
            ...
            {currentStep === 4 && (<Card className="p-8 shadow-xl min-h-[250px] flex flex-col items-center justify-center text-center">
                    <h2 className="text-2xl font-bold text-gray-900 mb-6">Processing Payment...</h2>
                    {loading ? (<>
                            <lucide_react_1.Loader2 className="w-10 h-10 text-orange-500 animate-spin mb-4"/>
                            <p className="text-gray-600">Please wait while we secure your purchase.</p>
                        </>) : error ? (<>
                            <lucide_react_1.AlertCircle className="w-10 h-10 text-red-500 mb-4"/>
                            <p className="text-red-700 font-semibold">{error}</p>
                            <Button onClick={() => setCurrentStep(3)} className="mt-4 bg-gray-500 hover:bg-gray-600">Try Payment Again</Button>
                        </>) : (
            // This state should not be hit if orderPlaced is true, but for completeness:
            <p className="text-gray-600">Moving to confirmation...</p>)}
                </Card>)}

            {/* Global Error Message */}
            {error && currentStep !== 4 && (<div className="mt-6 p-4 bg-red-50 border border-red-300 rounded-xl flex items-center gap-3 shadow-sm">
                <lucide_react_1.AlertCircle className="w-5 h-5 text-red-600 flex-shrink-0"/>
          ...
          <div className="lg:col-span-1">
            <Card className="p-6 sticky top-8 bg-white border-2 border-orange-500 shadow-2xl">
              ...
              {/* Actions */}
              <Button onClick={handlePlaceOrder} disabled={loading || currentStep === 4} className="w-full bg-orange-600 hover:bg-orange-700 text-white font-extrabold text-lg py-3 shadow-lg hover:shadow-xl">
                {loading ? (<span className="flex items-center gap-2"><lucide_react_1.Loader2 className="w-5 h-5 animate-spin"/>Processing...</span>) :
            currentStep === 1 ? ("Proceed to Billing") :
              ...
              {/* Back Button */}
              {currentStep > 1 && currentStep < 4 && (<Button onClick={() => setCurrentStep(currentStep - 1)} variant="outline" className="w-full mt-3 border-gray-300 text-gray-700 hover:bg-gray-100">
...
const DetailItem = ({ label, value, highlight = false }) => (<div>
...
const CurrencyButton = ({ currency, price, selected, onClick }) => {
    const symbol = currency === "USD" ? "$" : "Le ";
    const displayPrice = price.toFixed(currency === "USD" ? 2 : 0);
    return (<button onClick={onClick} className={`p-4 rounded-xl border-2 transition-all duration-300 text-center shadow-sm 
            ...
            <div className="flex items-center justify-center gap-2 mb-2">
                <lucide_react_1.DollarSign className={`w-5 h-5 ${selected ? "text-orange-600" : "text-gray-500"}`}/>
...
const PaymentMethodButton = ({ method, icon, description, selected, onClick }) => (<button onClick={onClick} className={`w-full p-4 rounded-xl border-2 transition-all duration-300 text-left shadow-sm 
            ${selected ? "border-orange-600 bg-orange-50 shadow-md ring-4 ring-orange-100" : "border-gray-200 bg-white hover:border-orange-300 hover:shadow-md"}`}>
        <div className="flex items-center gap-4">
            ...
            {selected && <lucide_react_1.CheckCircle2 className="w-5 h-5 text-orange-600"/>}
        </div>
    </button>);
// Helper for form groups
const FormGroup = ({ label, field, value, onChange, placeholder, type = 'text', required, disabled = false }) => (<div>
        <label className="block text-sm font-medium text-gray-700 mb-1">
            {label} {required && <span className="text-red-500">*</span>}
        </label>
        <input_1.Input type={type} value={value} onChange={(e) => onChange(field, e.target.value)} placeholder={placeholder} disabled={disabled} className={disabled ? "bg-gray-100 cursor-not-allowed" : ""}/>
    </div>);
